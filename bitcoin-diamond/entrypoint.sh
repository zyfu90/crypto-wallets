...
if [ "${ENV}" == "mainnet" ];
then
    /root/binary/bin/bitcoindiamondd -rescan -datadir=/root/mainnet_data -conf=/root/template.conf
else
    /root/binary/bin/bitcoindiamondd -rescan -regtest -datadir=/root/testnet_data -conf=/root/template.conf
fi