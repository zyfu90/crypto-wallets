...
if [ "${ENV}" == "mainnet" ];
then
    /root/binary/bin/bgoldd -rescan -datadir=/root/mainnet_data -conf=/root/template.conf
else
    /root/binary/bin/bgoldd -rescan -regtest -datadir=/root/testnet_data -conf=/root/template.conf
fi